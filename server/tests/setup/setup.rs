...
use odoo_ls_server::{
    core::{
        config::{ConfigEntry, DiagMissingImportsMode},
        entry_point::EntryPointMgr,
        odoo::SyncOdoo,
    },
    threads::SessionInfo,
    utils::PathSanitizer as _,
};
...
pub fn setup_server(with_odoo: bool) -> (SyncOdoo, ConfigEntry) {
    ...
    server
        .get_file_mgr()
        .borrow_mut()
        .add_workspace_folder(S!("test_addons_path"), test_addons_path.sanitize());
        ...
        match msg {
            Message::Notification(n) => {
                if n.method == PublishDiagnostics::METHOD {
                    let params: PublishDiagnosticsParams = serde_json::from_value(n.params)
                        .expect("Unable to parse PublishDiagnosticsParams");
...
pub fn get_diagnostics_for_paths(
...
pub fn get_diagnostics_test_comments(
    session: &mut SessionInfo,
    path: &str,
) -> Vec<(u32, Vec<String>)> {
    ...
    let file_info = file_mgr
        .get_file_info(&S!(path))
        .expect("File info not found");
