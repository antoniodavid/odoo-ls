...
impl SyncOdoo {
    ...
    fn load_file_cache(session: &mut SessionInfo) {
        ...
        if let Some(cache) = cache_manager.load(&odoo_path) {
            info!(
                "Loaded file metadata cache with {} entries",
                cache.files.len()
            );
    ...
    fn save_module_cache(session: &mut SessionInfo) {
        if let Some(module_cache_manager) = &session.sync_odoo.module_cache_manager {
            ...
            let modules_to_cache: Vec<_> = session
                .sync_odoo
                .modules
                .values()
            ...
            for module_rc in modules_to_cache {
                let cached_module = module_rc
                    .borrow()
                    .as_module_package()
                    .to_cached_module(session);
                        ...
                        match item {
                            Ok(item) => {
                                    ...
                                    ))
                                {
                                    ...
                                    let odoo_path = session
                                        .sync_odoo
                                        .config
                                        .odoo_path
                                        .clone()
                                        .unwrap_or_default();
                                    if let Some(cache_manager) =
                                        &session.sync_odoo.module_cache_manager
                                    {
                                        if let Some(cached_module) = cache_manager.load_module(
                                            &item.file_name().to_str().unwrap().to_string(),
                                            &odoo_path,
                                        ) {
                                            // Verify cache validity by checking file mtimes
                                            let is_valid = cached_module.file_hashes.iter().all(
                                                |(path, mtime)| {
                                                    if let Some(meta) =
                                                        get_file_metadata(Path::new(path))
                                                    {
                                                        meta.mtime == *mtime
                                                    } else {
                                                        false
                                            ...
                                            if is_valid
                                                && cached_module.path == item.path().sanitize()
                                            {
                                                // Reconstruct module from cache
                                                if let Some(module) =
                                                    ModuleSymbol::from_cached_module(&cached_module)
                                                {
                                                    // We need to wrap it in Symbol::Package and register it
                                                    let module_rc =
                                                        Rc::new(RefCell::new(Symbol::Package(
                                                            PackageSymbol::Module(module),
                                                        )));

                                                    // Register in session modules
                                                    session.sync_odoo.modules.insert(
                                                        oyarn!("{}", cached_module.dir_name),
                                                        Rc::downgrade(&module_rc),
                                                    );

                                                    // Add to addons symbol (parent)
                                                    match *addons_symbol.borrow_mut() {
                                                        Symbol::Package(ref mut p) => {
                                                            p.add_file(&module_rc)
                                                        }
                                                        Symbol::DiskDir(ref mut d) => {
                                                            d.add_file(&module_rc)
                                                        }
                                                        Symbol::Namespace(ref mut n) => {
                                                            n.add_file(&module_rc)
                                                        }
                                                        _ => warn!(
                                                            "Addons symbol is not a container"
                                                        ),
                                                    }
                                                    ...
                                                    module_rc
                                                        .borrow_mut()
                                                        .set_weak_self(Rc::downgrade(&module_rc));
                                                    module_rc.borrow_mut().set_parent(Some(
                                                        Rc::downgrade(&addons_symbol),
                                                    ));

                                                    // Hydrate models and files from cache with ARCH_EVAL support
                                                    ModuleSymbol::populate_models_from_cache_static(
                                                        session,
                                                        &cached_module,
                                                        module_rc.clone(),
                                                    );
                                                    ModuleSymbol::populate_files_from_cache_static(
                                                        session,
                                                        &cached_module,
                                                        module_rc.clone(),
                                                    );

                                                    {
                                                        ...
                                                        let entry =
                                                            session.sync_odoo.get_main_entry();
                                                        let module_sym =
                                                            module_ref.as_module_package();
                                                        let count = module_sym.data_symbols.len();
                                                        for (path, file_rc) in
                                                            module_sym.data_symbols.iter()
                                                        {
                                                            entry.borrow_mut().data_symbols.insert(
                                                                path.clone(),
                                                                Rc::downgrade(file_rc),
                                                            );
                                                            session
                                                                .sync_odoo
                                                                .add_to_rebuild_arch_eval(
                                                                    file_rc.clone(),
                                                                );
                                                        }
                                                        info!("Registered {} files from cache for module {}", count, item.file_name().to_str().unwrap());
                                                    }
