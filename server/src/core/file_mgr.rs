...
use lsp_types::{
    Diagnostic, DiagnosticSeverity, MessageType, NumberOrString, Position,
...
impl AstCacheManager {
    ...
    pub fn demote_to_transient(&mut self, path: &str) {
        if let Some(module) = self.persistent.remove(path) {
            self.transient
                .insert(path.to_string(), Arc::downgrade(&module));
        }
    }
...
pub struct FileInfoAst {
    ...
    pub js_module_info: Option<Arc<JsModuleInfo>>,
...
impl FileInfoAst {
    pub fn get_stmts(&self) -> Option<&Vec<Stmt>> {
        self.indexed_module
            .as_ref()
            .map(|module| &module.parsed.syntax().body)
    }

    pub fn evict(&mut self) {
        self.indexed_module = None;
        self.js_module_info = None;
    }

    pub fn is_evicted(&self) -> bool {
        match self.ast_type {
            AstType::Python => self.indexed_module.is_none(),
            AstType::Js => self.js_module_info.is_none(),
            _ => false,
        }
    }
}
...
impl FileInfo {
    ...
    pub fn update(
        ...
        match version {
            ...
            Some(-100) => {
                if !self.opened {
                    ...
                    // If opened, with -100, we do not update
                    return false;
                }
            }
        ...
        if let Some(content) = content {
            ...
            if content.len() == 1 && content[0].range.is_none() {
                self.file_info_ast.borrow_mut().text_document = Some(TextDocument::new(
                    content[0].text.clone(),
                    self.version.expect("Expected version on file did Open"),
                ));
            } else {
                self.file_info_ast
                    .borrow_mut()
            ...
            session.log_message(
                MessageType::ERROR,
                format!("Attempt to update untitled file {}, without changes", path),
            );
            return false;
        } else {
            match fs::read_to_string(path) {
                Ok(content) => {
                    self.file_info_ast.borrow_mut().text_document =
                        Some(TextDocument::new(content, self.version.unwrap_or(-1)));
                }
                Err(e) => {
                    session.log_message(
                        MessageType::ERROR,
    ...
    pub fn _build_ast(&mut self, session: &mut SessionInfo, is_external: bool) {
        ...
        if self.uri.ends_with(".js") {
            self.file_info_ast.borrow_mut().ast_type = AstType::Js;
            let fia_rc = self.file_info_ast.clone();
            let source = S!(fia_rc.borrow().text_document.as_ref().unwrap().contents());
            if let Some(info) = crate::core::js_parser::JsParser::parse(&source) {
                self.file_info_ast.borrow_mut().js_module_info = Some(Arc::new(info));
            }
            return;
        }
        ...
        let parsed_module =
            ruff_python_parser::parse_unchecked_source(source.as_str(), python_source_type);
        if !is_external {
        ...
        for error in parsed_module.errors().iter() {
            ...
            if let Some(diagnostic_base) =
                create_diagnostic(&session, DiagnosticCode::OLS01000, &[])
            {
    ...
    pub fn prepare_ast(&mut self, session: &mut SessionInfo) {
        ...
        if self.file_info_ast.borrow_mut().text_document.is_none() {
            //can already be set in xml files
        ...
        self.file_info_ast
            ...
            .hash(&mut hasher);
        self.file_info_ast.borrow_mut().text_hash = hasher.finish();
        let is_in_workspace = session
            .sync_odoo
            .get_file_mgr()
            .borrow()
            .is_in_workspace(&self.uri);
        ...
        if let Some(module) = &self.file_info_ast.borrow().indexed_module {
            ...
            if self.opened {
                file_mgr
                    ...
                    .ast_cache
                    .borrow_mut()
                    .insert_persistent(self.uri.clone(), module.clone());
            } else {
                file_mgr
                    .borrow()
                    .ast_cache
                    .borrow_mut()
                    .insert_transient(self.uri.clone(), module.clone());
            }
        }
    }
                ...
                TokenKind::Comment => {
                        ...
                        || text.starts_with("# odools: noqa")
                    {
                        ...
                        if let Some(after_noqa) = after_noqa {
                            ...
                            let source_location = text_doc.index().source_location(
                                token.start(),
                                text_doc.contents(),
                                encoding,
                            );
                            if let Some(previous_token) = previous_token {
                                let prev_location = file_info_ast_ref
                                    .text_document
                                    .as_ref()
                                    .unwrap()
                                    .index()
                                ...
                                if prev_location.line == source_location.line {
                                    self.noqas_lines.insert(
                                        source_location.line.to_zero_indexed() as u32,
                                        noqa_to_add.unwrap(),
                                    );
                                    noqa_to_add = None;
                                    continue;
                    ...
                    if parse_test_comments {
                        if text.starts_with("#OLS") || text.starts_with("# OLS") {
                            ...
                            self.diag_test_comments
                                .push((source_location.line.to_zero_indexed() as u32, codes));
                        }
                    }
                }
    ...
    fn add_noqa_bloc(&mut self, index: u32, noqa_to_add: NoqaInfo) {
        if let Some(noqa_bloc) = self.noqas_blocs.remove(&index) {
            self.noqas_blocs
                .insert(index, combine_noqa_info(&vec![noqa_bloc, noqa_to_add]));
        } else {
    ...
    pub fn update_validation_diagnostics(
        &mut self,
        diagnostics: HashMap<BuildSteps, Vec<Diagnostic>>,
    ) {
        self.need_push = true;
    ...
    pub fn update_diagnostic_filters(&mut self, session: &SessionInfo) {
        self.diagnostic_filters = session
            .sync_odoo
            .config
            .diagnostic_filters
            .iter()
            .cloned()
            .filter(|filter| match filter.path_type {
                DiagnosticFilterPathType::In => filter.paths.iter().any(|p| p.matches(&self.uri)),
    ...
    pub fn publish_diagnostics(&mut self, session: &mut SessionInfo) {
        if self.need_push {
            ...
            'diagnostics: for d in self.diagnostics.values().flatten() {
                ...
                if let Some(noqa_line) = self.noqas_lines.get(&updated_line) {
                    match noqa_line {
                        ...
                        NoqaInfo::Codes(codes) => match &updated.code {
                            None => {
                                continue;
                            }
            ...
            session.send_notification::<PublishDiagnosticsParams>(
    ...
    fn offset_to_position_with_text_document(
        text_document: &TextDocument,
        offset: u32,
        encoding: PositionEncoding,
    ) -> Position {
        let location = text_document.index().source_location(
            offset.into(),
    ...
    fn try_offset_to_position_with_text_document(
        text_document: &TextDocument,
        offset: u32,
        encoding: PositionEncoding,
    ) -> Option<Position> {
    ...
    pub fn offset_to_position(&self, offset: u32, encoding: PositionEncoding) -> Position {
        FileInfo::offset_to_position_with_text_document(
            self.file_info_ast
                .borrow()
                .text_document
                .as_ref()
                .expect("no text_document provided"),
            offset,
            encoding,
        )
    }
    ...
    fn try_offset_to_position(&self, offset: u32, encoding: PositionEncoding) -> Option<Position> {
        FileInfo::try_offset_to_position_with_text_document(
            self.file_info_ast.borrow().text_document.as_ref()?,
            offset,
            encoding,
        )
    }
    ...
    pub fn try_text_range_to_range(
        &self,
        range: &TextRange,
        encoding: PositionEncoding,
    ) -> Option<Range> {
    ...
    fn position_to_offset_with_text_document(
        text_document: &TextDocument,
        line: u32,
        char: u32,
        encoding: PositionEncoding,
        ...
        text_document
            .index()
            .offset(position, text_document.contents(), encoding)
            .into()
    }

    pub fn position_to_offset(&self, line: u32, char: u32, encoding: PositionEncoding) -> usize {
        FileInfo::position_to_offset_with_text_document(
            self.file_info_ast
                .borrow()
                .text_document
                .as_ref()
...
impl FileMgr {
    ...
    pub fn text_range_to_range(
        ...
        if let Some(file) = file {
            ...
            return file
                .borrow()
                .text_range_to_range(range, session.sync_odoo.encoding);
        }
        // For untitled, never try to read from disk
        if Self::is_untitled(path) {
            session.log_message(
                MessageType::ERROR,
                format!("Untitled file {} not found in memory", path),
            );
        ...
        match fs::read_to_string(path) {
            Ok(content) => {
                ...
                return Range {
                    start: FileInfo::offset_to_position_with_text_document(
                        &text_document,
                        range.start().into(),
                        session.sync_odoo.encoding,
                    ),
                    end: FileInfo::offset_to_position_with_text_document(
                        &text_document,
                        range.end().into(),
                        session.sync_odoo.encoding,
                    ),
            ...
            Err(_) => {
                session.log_message(MessageType::ERROR, format!("Failed to read file {}", path))
            }
        };
        Range::default()
    }
    ...
    pub fn std_range_to_range(
        ...
        if let Some(file) = file {
            ...
            return file
                .borrow()
                .std_range_to_range(range, session.sync_odoo.encoding);
        }
        // For untitled, never try to read from disk
        if Self::is_untitled(path) {
            session.log_message(
    ...
    pub fn update_file_info(
        ...
        if (version.is_some() && version.unwrap() != -100) || !file_info.borrow().opened || force {
            ...
            let is_part_of_ep = ep_mgr
                .iter_all_but_public()
                .any(|entry| uri.starts_with(&entry.borrow().path));
            drop(ep_mgr);
            updated = file_info_mut.update(
                session,
                uri,
                content,
                version,
            ...
            if updated {
                if let Some(module) = &file_info.borrow().file_info_ast.borrow().indexed_module {
                    ...
                    if opened {
                        self.ast_cache
                            .borrow_mut()
                            .insert_persistent(uri.to_string(), module.clone());
                    } else {
                        self.ast_cache
                            .borrow_mut()
                            .insert_transient(uri.to_string(), module.clone());
                    }
                }
            }
        }
    ...
    pub fn create_file_info_for_cached(
        &mut self,
        path: &str,
        processed_text_hash: u64,
    ) -> Rc<RefCell<FileInfo>> {
    ...
    pub fn delete_path(session: &mut SessionInfo, uri: &String) {
        ...
        let matching_keys: Vec<String> = session
            .sync_odoo
            .get_file_mgr()
            .borrow_mut()
            .files
            .keys()
            .filter(|k| PathBuf::from(k).starts_with(uri))
            .cloned()
            .collect();
