...
use lsp_types::{
    notification::{
        ...
        DidOpenTextDocument, DidRenameFiles, DidSaveTextDocument, Notification,
    },
    request::{
        Completion, DocumentSymbolRequest, GotoDefinition, HoverRequest, References, Request,
        ResolveCompletionItem, Shutdown, WorkspaceSymbolRequest, WorkspaceSymbolResolve,
    },
    CancelParams, CompletionOptions, DefinitionOptions, DocumentSymbolOptions, FileOperationFilter,
    FileOperationPattern, FileOperationRegistrationOptions, HoverProviderCapability,
    InitializeParams, InitializeResult, OneOf, ReferencesOptions, SaveOptions, ServerCapabilities,
...
use crate::{
    constants::{DEBUG_THREADS, EXTENSION_VERSION},
    core::{file_mgr::FileMgr, odoo::SyncOdoo},
    crash_buffer,
    threads::{
        delayed_changes_process_thread, message_processor_thread_main, DelayedProcessingMessage,
    },
    S,
};
...
impl Server {
    pub fn new_tcp() -> Result<Self, Error> {
        match Connection::listen("127.0.0.1:2087") {
            Ok((conn, io_threads)) => Ok(Server::init(conn, io_threads)),
            Err(e) => Err(e),
        }
    }
    ...
    fn init(conn: Connection, _io_threads: IoThreads) -> Self {
        ...
        let main_thread = {
            ...
            std::thread::spawn(move || {
                message_processor_thread_main(
                    sync_odoo,
                    generic_receiver_s_to_main,
                    sender_main_to_s.clone(),
                    receiver_s_to_main.clone(),
        ...
        let delayed_process_thread = std::thread::spawn(move || {
            delayed_changes_process_thread(
                sender_delayed_to_s,
                receiver_s_to_delayed,
                receiver_delayed_process,
                so,
                delayed_process_sender_to_delayed_process,
            )
        });
        Self {
    ...
    pub fn initialize(&mut self) -> Result<(), ServerError> {
            ...
            capabilities: ServerCapabilities {
                text_document_sync: Some(TextDocumentSyncCapability::Options(
                    TextDocumentSyncOptions {
                        ...
                        )), //TODO could deactivate if set on 'afterDelay?
                    },
                )),
                ...
                position_encoding: Some(match self.sync_odoo.lock().unwrap().encoding {
        ...
        self.connection
            .as_ref()
            .unwrap()
            .initialize_finish(id, serde_json::to_value(initialize_data).unwrap())?;
        let _ = self
            .connection
            .as_ref()
            .unwrap()
        ...
        self.req_sender_s_to_main
            .send(Message::Notification(lsp_server::Notification {
                method: S!("custom/server/register_capabilities"),
                params: serde_json::Value::Null,
            }))
            .unwrap();
        self.req_sender_s_to_main
            .send(Message::Notification(lsp_server::Notification {
                method: S!("custom/server/init"),
                params: serde_json::Value::Null,
            }))
            .unwrap();
        Ok(())
    ...
    fn shutdown_threads(&mut self, message: &str) {
        self.terminate_rebuild_boolean
        ...
        self.req_sender_s_to_main
            .send(shutdown_notification.clone())
            .unwrap();
    ...
    pub fn run(mut self, client_pid: Option<u32>) -> bool {
            ...
            if index == 0 {
                ...
                if let Message::Request(r) = &msg {
                    ...
                    if n.method == "exit" {
                        ...
                        let cancel_params =
                            serde_json::from_value::<CancelParams>(n.params.clone()).unwrap();
                        let req_id: RequestId = match cancel_params.id {
                            lsp_types::NumberOrString::Number(id) => id.into(),
                            lsp_types::NumberOrString::String(id) => id.into(),
                        };
                        self.running_request_ids
                            .lock()
                            .unwrap()
                            .retain(|id| id != &req_id);
                        continue;
        ...
        let _ = self
            .sender_to_delayed_process
            .send(DelayedProcessingMessage::EXIT);
        ...
        match msg {
            Message::Request(r) => {
                ...
                match r.method.as_str() {
                    ...
                    | Completion::METHOD => {
                        self.interrupt_rebuild_boolean
                            .store(true, std::sync::atomic::Ordering::SeqCst);
                        if DEBUG_THREADS {
            ...
            loop {
                ...
                match nix::sys::wait::waitpid(pid, Some(nix::sys::wait::WaitPidFlag::WNOHANG)) {
                    Ok(nix::sys::wait::WaitStatus::Exited(_, status)) => {
                        warn!(
                            "Process {} exited with status {} - killing extension in 10 secs",
                            pid, status
                        );
                    ...
                    Ok(nix::sys::wait::WaitStatus::Signaled(_, signal, _)) => {
                        warn!(
                            "Process {} was killed by signal {:?} - killing extension in 10 secs",
                            pid, signal
                        );
                        std::thread::sleep(std::time::Duration::from_secs(10));
                        exit(1);
